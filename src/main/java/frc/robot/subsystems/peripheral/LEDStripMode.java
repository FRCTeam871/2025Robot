...
public class LEDStripMode {
    ...
    public static TeensyPacket chase(int offset, int period, int change, int color1, int color2) {
        return new TeensyPacket("CHASE" + Teensy.DELIMITER + offset + Teensy.DELIMITER + period + Teensy.DELIMITER
                + change + Teensy.DELIMITER + Teensy.hex(color1) + Teensy.DELIMITER + Teensy.hex(color2));
    }
    ...
    public static TeensyPacket wave(int color1, int color2, int height, int period) {
        return new TeensyPacket("WAVE" + Teensy.DELIMITER + Teensy.hex(color1) + Teensy.DELIMITER + Teensy.hex(color2)
                + Teensy.DELIMITER + height + Teensy.DELIMITER + period);
    }
    ...
    public static TeensyPacket fade(int color1, int color2, int period, int offset) {
        return new TeensyPacket("FADE" + Teensy.DELIMITER + Teensy.hex(color1) + Teensy.DELIMITER + Teensy.hex(color2)
                + Teensy.DELIMITER + period + Teensy.DELIMITER + offset);
    }
    ...
    public static TeensyPacket binary(int num, int color1, int color2, int countDelay) {
        return new TeensyPacket("BINARY" + Teensy.DELIMITER + num + Teensy.DELIMITER + Teensy.hex(color1)
                + Teensy.DELIMITER + Teensy.hex(color2) + Teensy.DELIMITER + countDelay);
    }
}
